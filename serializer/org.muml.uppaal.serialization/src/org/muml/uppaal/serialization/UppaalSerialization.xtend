...
class UppaalSerialization {
    ...
    def function(
        Function it) '''«typedefinition(returnType)» «name»(«FOR i : parameter ?:emptyList SEPARATOR ', '»«parameter(i)»«ENDFOR») «block(block)»'''
    ...
    def dispatch declaration(
        TypeDeclaration it) '''typedef «typedefinition(typeDefinition)» «FOR DeclaredType i : type ?:emptyList SEPARATOR ', '»«typedeclaration(i)»«ENDFOR»;'''
    ...
    def declaration(VariableDeclaration it,
        Boolean callByReference) '''«prefix(it)»«typedefinition(typeDefinition)» «IF callByReference»&amp;«ENDIF»«FOR i : variable ?:emptyList SEPARATOR ', '»«variable(i)»«ENDFOR»'''

    def dispatch prefix(ClockVariableDeclaration it) ''''''

    def dispatch prefix(
        DataVariableDeclaration it) '''«IF prefix != DataVariablePrefix::NONE»«prefix.toString()» «ENDIF»'''

    def dispatch prefix(ChannelVariableDeclaration it) '''«IF broadcast»broadcast «ENDIF»«IF urgent»urgent «ENDIF»'''

    def dispatch declaration(
        TemplateDeclaration it) '''«name(declaredTemplate)»«IF declaredTemplate.parameter.size > 0»(«FOR i : declaredTemplate.parameter ?:emptyList SEPARATOR ', '»«parameter(i)»«ENDFOR»)«ENDIF» = «name(declaredTemplate.referredTemplate)»(«FOR i : argument ?:emptyList SEPARATOR ', '»«expression(i)»«ENDFOR»);'''

    def dispatch channelPriority(
        ChannelPriority it) '''chan priority «FOR i : item ?:emptyList SEPARATOR ' < '»«item(i)»«ENDFOR»;'''
    ...
    def dispatch item(
        ChannelList it) '''«FOR IdentifierExpression i : channelExpression ?:emptyList SEPARATOR ', '»«expression(i)»«ENDFOR»'''
    ...
    def dispatch typedefinition(
        RangeTypeSpecification it) '''«baseType.toString()»[«expression(bounds.lowerBound)»,«expression(bounds.upperBound)»]'''
    ...
    def variable(
        Variable it) '''«name(it)»«FOR i : index ?:emptyList»«index(i)»«ENDFOR»«IF initializer != null» = «initializer(initializer)»«ENDIF»'''
    ...
    def dispatch initializer(
        ArrayInitializer it) '''{«FOR i : initializer ?:emptyList SEPARATOR ','»«initializer(i)»«ENDFOR»}'''
    ...
    def edge(Edge it) '''
        <transition «IF colorToString(it) != null»color="«colorToString(it)»"«ENDIF» >
    ...
    /* NOTE: Added by E. Postma */
    def colorToString(Edge it) {
        if (color == null)
            return null;
        switch (color) {
            case ColorKind.BLACK: return "#000000"
    ...
    def synchronization(
        Synchronization it) '''«expression(channelExpression)»«IF kind == SynchronizationKind::RECEIVE»?«ELSE»!«ENDIF»'''
    ...
    def dispatch statement(
        ForLoop it) '''for(«expression(initialization)»; «expression(condition)»; «expression(iteration)») «IF !(statement instanceof Block)»
...
«IF elseStatement != null»else «IF !(elseStatement instanceof Block)»
    ...
    def dispatch statement(
        ReturnStatement it) '''return «IF returnExpression != null»«expression(returnExpression)»«ENDIF»;'''

    /* expressions */
    // Creates parentheses around the supplied expression if it has lower precedence than its parent
    def expressionOptionalParentheses(
        Expression it) '''«IF eContainer() instanceof Expression && precedence(it) < precedence(eContainer() as Expression)»(«expression(it)»)«ELSE»«expression(it)»«ENDIF»'''
    ...
    def expressionOptionalParenthesesNonAssoc(
        Expression it) '''«IF eContainer() instanceof Expression && (precedence(it) < precedence(eContainer() as Expression) || precedence(it) == precedence(eContainer() as Expression) && !isAssociative(eContainer() as Expression))»(«expression(it)»)«ELSE»«expression(it)»«ENDIF»'''

    def dispatch expression(Void it) ''''''

    def dispatch expression(
        AssignmentExpression it) '''«expressionOptionalParentheses(firstExpr)» «assignment(operator)» «expressionOptionalParentheses(secondExpr)»'''

    def dispatch expression(
        IdentifierExpression it) '''«name(identifier)»«IF !index.isEmpty»[«FOR i : index ?:emptyList SEPARATOR ']['»«expression(i)»«ENDFOR»]«ENDIF»'''

    def dispatch expression(NegationExpression it) '''not «expressionOptionalParentheses(negatedExpression)»'''

    def dispatch expression(
        FunctionCallExpression it) '''«function.name»(«FOR i : argument ?:emptyList SEPARATOR ', '»«expression(i)»«ENDFOR»)'''

    def dispatch expression(
        LogicalExpression it) '''«expressionOptionalParentheses(firstExpr)» «operator.toString()» «expressionOptionalParenthesesNonAssoc(secondExpr)»'''

    def dispatch expression(
        CompareExpression it) '''«expressionOptionalParentheses(firstExpr)» «comparison(operator)» «expressionOptionalParenthesesNonAssoc(secondExpr)»'''

    def dispatch expression(
        ArithmeticExpression it) '''«expressionOptionalParentheses(firstExpr)»«arithmetic(operator)»«expressionOptionalParenthesesNonAssoc(secondExpr)»'''

    def dispatch expression(LiteralExpression it) '''«text»'''

    def dispatch expression(
        IncrementDecrementExpression it) '''«IF position==IncrementDecrementPosition::PRE»«operator(operator)»«ENDIF»«expressionOptionalParentheses(expression)»«IF position==IncrementDecrementPosition::POST»«operator(operator)»«ENDIF»'''

    def operator(
        IncrementDecrementOperator it) '''«IF it==IncrementDecrementOperator::INCREMENT»++«ENDIF»«IF it==IncrementDecrementOperator::DECREMENT»--«ENDIF»'''

    def dispatch expression(
        ConditionExpression it) '''«expressionOptionalParenthesesNonAssoc(ifExpression)» ? «expressionOptionalParentheses(thenExpression)» : «expressionOptionalParentheses(elseExpression)»'''

    def dispatch expression(
        ScopedIdentifierExpression it) '''«expressionOptionalParentheses(scope)».«expressionOptionalParentheses(identifier)»'''

    def dispatch expression(
        QuantificationExpression it) '''«quantifier(quantifier)» («variable(variable.get(0))» : «typedefinition(typeDefinition)») «expressionOptionalParentheses(expression)»'''

    def quantifier(
        Quantifier it) '''«IF it==Quantifier::UNIVERSAL»forall«ENDIF»«IF it==Quantifier::EXISTENTIAL»exists«ENDIF»'''
    ...
    def dispatch expression(
        MinMaxExpression it) '''«expressionOptionalParentheses(firstExpr)» «IF operator == MinMaxOperator::MIN»<?«ENDIF»«IF operator == MinMaxOperator::MAX»>?«ENDIF» «expressionOptionalParenthesesNonAssoc(secondExpr)»'''

    def dispatch expression(
        BitwiseExpression it) '''«expressionOptionalParentheses(firstExpr)» «IF operator==BitwiseOperator::AND»&«ENDIF»«IF operator==BitwiseOperator::OR»|«ENDIF»«IF operator==BitwiseOperator::XOR»^«ENDIF» «expressionOptionalParentheses(secondExpr)»'''
    ...
    /* define system */
    def system(
        System it) '''system «FOR i : instantiationList ?:emptyList SEPARATOR ' < '»«instantiationList(i)»«ENDFOR»;'''

    def instantiationList(
        InstantiationList it) '''«FOR AbstractTemplate i : template ?:emptyList SEPARATOR ','»«name(i)»«ENDFOR»'''

    def progressMeasure(
        ProgressMeasure it) '''progress {«FOR i : expression ?:emptyList SEPARATOR ';'»«expression(i)»«ENDFOR»;}'''
    ...
    def boolean isAssociative(Expression it) {
        switch (it) {
            ArithmeticExpression: return operator == ArithmeticOperator::MULTIPLICATE ||
                operator == ArithmeticOperator::ADD // left-associative
    ...
    def int precedence(Expression it) {
        switch (it) { // listing 
            case it == null:
                return 1000
            case it instanceof IdentifierExpression:
                return 500
            case it instanceof ScopedIdentifierExpression:
                return 500
            case it instanceof LiteralExpression:
                return 500
            case it instanceof FunctionCallExpression:
                return 500
            case it instanceof IncrementDecrementExpression:
                return 300
            case it instanceof PlusExpression:
                return 300
            case it instanceof MinusExpression:
                return 300
            ...
            case it instanceof ArithmeticExpression && (
                ...
                return 280
            case it instanceof BitShiftExpression:
                return 270
            case it instanceof MinMaxExpression:
                return 260
            case it instanceof CompareExpression &&
                ((it as CompareExpression).operator == CompareOperator::EQUAL ||
                    (it as CompareExpression).operator == CompareOperator::UNEQUAL):
                return 250
            case it instanceof CompareExpression:
                return 245
            case it instanceof BitwiseExpression && (it as BitwiseExpression).operator == BitwiseOperator::AND:
                return 240
            case it instanceof BitwiseExpression && (it as BitwiseExpression).operator == BitwiseOperator::XOR:
                return 230
            case it instanceof BitwiseExpression && (it as BitwiseExpression).operator == BitwiseOperator::OR:
                return 220
            case it instanceof ConditionExpression:
                return 210
            case it instanceof AssignmentExpression:
                return 200
            case it instanceof NegationExpression:
                return 190
            case it instanceof LogicalExpression && (it as LogicalExpression).operator == LogicalOperator::AND:
                return 180
            case it instanceof LogicalExpression && (it as LogicalExpression).operator == LogicalOperator::OR:
                return 170
            case it instanceof LogicalExpression && (it as LogicalExpression).operator == LogicalOperator::IMPLY:
                return 170
            case it instanceof QuantificationExpression:
                return 160
        }
